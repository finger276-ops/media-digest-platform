"""Прежняя классификация микротемы — эталон для tests/test_message_normalize.py.

Это src/services/microtopics.py в том виде, в каком он был до ускорения
(коммит e4cfe2c): каждое правило — any(re.search(шаблон, текст, re.I)).
Заморожен намеренно: тест сверяет с ним новую классификацию на случайных
текстах. Если правила меняют сознательно, правится и этот файл.
"""
...
from services.tag_parsing import label_microtopic, normalize_label, tag_set
from services.text_cleaning import normalize_spaces
