"""Прежняя сборка обсуждений — эталон для tests/test_discussion_build.py.

Это make_discussions из src/services/discussion_build.py в том виде, в каком
он был до ускорения (коммит 2016ae6): построчный iterrows и отдельная
сортировка каждого обсуждения. Код заморожен намеренно и не правится вместе
с рабочим: тест сверяет с ним новую реализацию на случайных данных, и любое
расхождение — это изменение поведения, а не ускорение.

Если поведение сборки обсуждений меняют сознательно, правится и этот файл —
тогда в том же коммите видно, что именно поменялось.
"""
...
from services.tag_parsing import main_tag, source_topic_bucket_value, split_source_topics, tag_set
from services.text_cleaning import normalize_spaces, stable_hash
...
def make_discussions_reference(
