...
python tests/test_message_normalize.py        # разбор выгрузки: совпадает с прежним до байта, быстрее
...
python tests/test_clustering.py               # кластеризация инфоповодов без сюжетов (не Brand Analytics)
...
python tests/test_category_store.py           # выгрузка по категории: агрегаты брендов, хранение, блок загрузки
...
python tests/test_client_insights.py          # клиентский обзор: изменения к прошлому периоду, согласование рода
python tests/test_overview_ui.py              # «Обзор»: цвет карточек тональности
python tests/test_overview_previous_period.py # «Обзор»: сравнение с прошлым периодом не врёт
...
python tests/test_tag_hierarchy_ui.py         # загрузка структуры тегов: CSV/xlsx, починка битых стилей
...
python tests/test_access_codes.py             # коды доступа: один код — один проект, дубли закрывают вход
