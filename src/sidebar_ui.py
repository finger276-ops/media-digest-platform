...
def dashboard_view_mode_for_session(
    ...
    # клиентский вид почти ничего не прятал. Теперь он прячет правку целиком,
    # и прежний дефолт означал бы, что аналитик открывает проект без единого
    # рабочего инструмента и должен сам догадаться про переключатель.
    # Явная настройка проекта по-прежнему сильнее: владелец может назначить
    # клиентский вид по умолчанию, например для демо-проекта.
    default_mode = str(settings.get("default_view_mode") or "analyst")
    ...
    selected = st.sidebar.radio(
        ...
        key=key,
        help=(
            "Клиентский вид — предпросмотр кабинета заказчика: разделы "
            "выглядят так, как их увидит тот, у кого код просмотра. Правка "
            "инфоповодов и саммари, формулы методики, веса BPI, разметка "
            "брендов и тексты от ИИ скрываются. Данные не меняются, "
            "переключиться обратно можно в любой момент. Аналитический вид "
            "возвращает рабочие настройки."
        ),
    )
    return selected
