...
# текста). Перенос строк задаёт не сам stMetricLabel/stMetricValue/stMetricDelta,
# а вложенный <p> внутри stMarkdownContainer — именно там остаётся
# white-space: nowrap, если переопределить стиль только на внешнем блоке.
# Дельта (стрелка + число) подвержена тому же — длинные значения вроде
# "+64 000 (+17%)" резались точно так же, пока их не хватало отдельно.
...
def inject_metric_css() -> None:
    """Разрешить перенос строк в st.metric — вызывается один раз на страницу.

    render_metric_row делает это сам для своих карточек, но часть разделов
    (например, очередь автозагрузки, фильтр шума, список сессий) рисует
    st.metric напрямую через объект колонки, минуя этот модуль — без
    отдельного глобального вызова перенос строк для них не включался.
    """
