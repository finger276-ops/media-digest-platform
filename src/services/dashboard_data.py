...
привязываются инфоповоды, применяются ручные правки аналитика (тональность,
переносы между темами), вычищаются служебные теги и добавляются колонки для
расчётов. Любой расчёт, который берёт периоды сверх выбранных — динамика по
всем периодам, изменение к прошлому периоду, — должен проходить тот же путь.
Иначе одинаковые по данным периоды дают разные числа: NSS не видит
инфоповодов, а поправленная вручную тональность сравнивается с исходной.
"""
