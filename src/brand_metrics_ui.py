...
def render_metric_conclusions(
    ...
    *,
    ...
    то, что заказчик должен читать. Цифру он получает вместе с выводом — на
    экране и в CSV, в отчёт Word/PDF/PNG индексы не входят, — а устройство
    расчёта — предмет отдельного разговора, если спросит.
    ...
    st.caption(
        ...
        "Текст сохраняется для выбранного периода и скачивается вместе с "
        "индексами кнопкой «Скачать метрики в CSV». В отчёт Word/PDF/PNG из "
        "раздела «Отчёт» индексы и выводы не входят."
    )
